namespace SquirrelBytes.DOTS
{
	public static class DOTSUtil
	{
		...
		/// <param name="index">"for" index, used to defire the interaction direction</param>
		...
		public static bool SwitchAndTryInteract<T, Y>
			(this ref EntityPair pair, ComponentDataFromEntity<T> getterA, ComponentDataFromEntity<Y> getterB) where T : struct, IComponentData where Y : struct, IComponentData
		{
			pair.PairSwitch();
